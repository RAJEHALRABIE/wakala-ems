  ...
  useEffect(() => {
    if (client) {
      setForm({
        ...
        agencyDate: client.agencyDate ? new Date(client.agencyDate).toISOString().split("T")[0] : "",
        propertyDocType: (client.propertyDocType as PropertyDocType) || "Deed",
        deedNumber: client.deedNumber || "",
        deedDate: client.deedDate ? new Date(client.deedDate).toISOString().split("T")[0] : "",
        requestNumber: client.requestNumber || "",
        requestDate: client.requestDate ? new Date(client.requestDate).toISOString().split("T")[0] : "",
        ...
        decisionDate: client.decisionDate ? new Date(client.decisionDate).toISOString().split("T")[0] : "",
        ...
        return (
          <>
            <div className="space-y-2">
              ...
              <Input value={form.deedNumber} onChange={(e) => setForm({ ...form, deedNumber: e.target.value })} />
            </div>
            <DualDatePicker
              label="تاريخ الصك"
              value={form.deedDate}
        ...
        return (
          <>
            <div className="space-y-2">
              ...
              <Input value={form.requestNumber} onChange={(e) => setForm({ ...form, requestNumber: e.target.value })} />
            </div>
            <DualDatePicker
              label="تاريخ الطلب"
      ...
      case 'PARTIAL':
        return (
          <>
            ...
            <DualDatePicker label="تاريخ القرار" value={form.decisionDate} onChange={(date) => setForm({ ...form, decisionDate: date })} />
            <div className="space-y-2">
              <Label>المساحة المنزوعة (م²)</Label>
              <Input type="number" value={form.expropriatedArea} onChange={(e) => setForm({ ...form, expropriatedArea: e.target.value })} />
            </div>
            <div className="space-y-2">
              <Label>المساحة المتبقية (م²)</Label>
              <Input type="number" value={form.remainingArea} onChange={(e) => setForm({ ...form, remainingArea: e.target.value })} />
            </div>
          </>
        );
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <Card>
          ...
          <CardContent className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            ...
            <div className="space-y-2"><Label>الهاتف</Label><Input value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} type="tel" /></div>
            <div className="space-y-2"><Label>رقم الهوية</Label><Input value={form.idNumber} onChange={(e) => setForm({ ...form, idNumber: e.target.value })} /></div>
            <div className="space-y-2"><Label>الحالة</Label><Select value={form.status} onValueChange={(v) => setForm({ ...form, status: v as ClientStatus })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{CLIENT_STATUSES.map((s) => (<SelectItem key={s} value={s}>{STATUS_LABELS[s]}</SelectItem>))}</SelectContent></Select></div>
          </CardContent>
        </Card>

        <Card>
          <CardHeader><CardTitle>معلومات الوكالة</CardTitle></CardHeader>
          <CardContent className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            <div className="space-y-2"><Label>الوكيل</Label><Select value={form.agentId?.toString() || "none"} onValueChange={(v) => setForm({ ...form, agentId: v === "none" ? null : parseInt(v) })}><SelectTrigger><SelectValue placeholder="اختر الوكيل" /></SelectTrigger><SelectContent><SelectItem value="none">بدون وكيل</SelectItem>{agents?.map((a) => (<SelectItem key={a.id} value={a.id.toString()}>{a.name}</SelectItem>))}</SelectContent></Select></div>
            <div className="space-y-2"><Label>رقم الوكالة</Label><Input value={form.wakalahNumber} onChange={(e) => setForm({ ...form, wakalahNumber: e.target.value })} /></div>
            <DualDatePicker label="تاريخ الوكالة" value={form.agencyDate} onChange={(date) => setForm({ ...form, agencyDate: date })} />
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            <div className="space-y-2"><Label>نوع المستند</Label><Select value={form.propertyDocType} onValueChange={(v: PropertyDocType) => setForm({ ...form, propertyDocType: v })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{PROPERTY_DOC_TYPES.map((t) => (<SelectItem key={t.value} value={t.value}>{t.label}</SelectItem>))}</SelectContent></Select></div>
            {renderPropertyFields()}
            <div className="space-y-2"><Label>المدينة/الحي</Label><Input value={form.city} onChange={(e) => setForm({ ...form, city: e.target.value })} /></div>
            <div className="space-y-2 md:col-span-2"><Label>رابط الخريطة</Label><div className="flex gap-2"><Input value={form.mapLink} onChange={(e) => setForm({ ...form, mapLink: e.target.value })} placeholder="رابط خرائط جوجل" className="flex-1" /><Dialog open={mapDialogOpen} onOpenChange={setMapDialogOpen}><DialogTrigger asChild><Button type="button" variant="outline" className="shrink-0"><Map className="h-4 w-4 ml-2" />تحديد من الخريطة</Button></DialogTrigger><DialogContent className="max-w-4xl max-h-[90vh]"><DialogHeader><DialogTitle className="flex items-center gap-2"><MapPin className="h-5 w-5" />تحديد الموقع</DialogTitle></DialogHeader><div className="space-y-4"><GoogleMap height="450px" center={{ lat: 24.7136, lng: 46.6753 }} zoom={6} selectable={true} selectedPosition={selectedMapPosition} onPositionSelect={handleMapPositionSelect} showSearch={true} showSatelliteToggle={true} />{selectedMapPosition && (<div className="flex items-center justify-between p-3 bg-green-50 rounded-lg border border-green-200"><div className="flex items-center gap-2 text-green-700"><Check className="h-5 w-5" /><span>الإحداثيات: {selectedMapPosition.lat.toFixed(6)}, {selectedMapPosition.lng.toFixed(6)}</span></div><Button type="button" onClick={confirmMapSelection} className="bg-green-600 hover:bg-green-700">تأكيد</Button></div>)}</div></DialogContent></Dialog></div>{form.mapLink && (() => { const coords = extractCoordinates(form.mapLink); return coords ? (<div className="flex items-center gap-2 text-sm text-green-600"><Check className="h-4 w-4" /><MapPin className="h-4 w-4" /><span>الإحداثيات: {formatCoordinates(coords)}</span></div>) : (<div className="flex items-center gap-2 text-sm text-amber-600"><AlertTriangle className="h-4 w-4" /><span>لم يتم التعرف على الإحداثيات</span></div>);})()}</div>
          </CardContent>
        </Card>
        
        <Card>
          <CardHeader><CardTitle>تفاصيل النزع والتحسينات</CardTitle></CardHeader>
          <CardContent className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            <div className="space-y-2">
              <Label>نوع النزع</Label>
              <Select value={form.expropriationType} onValueChange={(v: ExpropriationType) => setForm({ ...form, expropriationType: v })}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>{EXPROPRIATION_TYPES.map((t) => (<SelectItem key={t.value} value={t.value}>{t.label}</SelectItem>))}</SelectContent>
              </Select>
            </div>
            ...
            {/* تم تحديث القسم ليناسب طلبك: 3 أعمدة مع وجود gap للفصل بين المربع والنص */}
            ...
            <div className="space-y-2">
              ...
              <Input type="number" value={form.improvementValue} onChange={(e) => setForm({ ...form, improvementValue: e.target.value })} placeholder="مبلغ تقديري"/>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {form.expropriationType !== 'IMPROVEMENTS_ONLY' && (
              <>
                <div className="space-y-2"><Label>إجمالي المساحة (م²)</Label><Input type="number" value={form.areaSqm} onChange={(e) => setForm({ ...form, areaSqm: e.target.value })} placeholder="المساحة الكلية" /></div>
                <div className="space-y-2"><Label>سعر المتر المتوقع (ريال)</Label><Input type="number" value={form.expectedCompensationPerSqm} onChange={(e) => setForm({ ...form, expectedCompensationPerSqm: e.target.value })} /></div>
              </>
            )}
            <div className="space-y-2"><Label>نسبة الاستحقاق (%)</Label><Input type="number" value={form.possessionRatio} onChange={(e) => setForm({ ...form, possessionRatio: e.target.value })} placeholder="1.0 = 100%" step="0.01" /></div>
            <div className="space-y-2"><Label>نسبة الأتعاب (%)</Label><Input type="number" value={form.baseFeePercentage} onChange={(e) => setForm({ ...form, baseFeePercentage: e.target.value })} placeholder="مثال: 2.5" /></div>
            <div className="space-y-2"><Label>نسبة تعويض إضافي (%)</Label><Input type="number" value={form.extraCompRate} onChange={(e) => setForm({ ...form, extraCompRate: e.target.value })} /></div>
            <div className="space-y-2"><Label>تعويض الضرر (ريال)</Label><Input type="number" value={form.damageToRemainingComp} onChange={(e) => setForm({ ...form, damageToRemainingComp: e.target.value })} /></div>
            <div className="space-y-2 lg:col-span-2"><Label>التعويض الرسمي (ريال)</Label><Input type="number" value={form.officialCompensationAmount} onChange={(e) => setForm({ ...form, officialCompensationAmount: e.target.value })} placeholder="اختياري" /></div>
          </CardContent>
        </Card>
        ...
        <Card className="border-blue-200 bg-blue-50/20">
            ...
            <CardContent className="space-y-4">
                {warnings.length > 0 && (
                    <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg space-y-1 text-sm">
                        <p className="font-semibold text-amber-800">تنبيهات:</p>
                        {warnings.map((w, i) => <p key={i} className="text-amber-700">- {w}</p>)}
                    </div>
                )}
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-4 gap-y-6">
                    <div className="space-y-1">
                        <dt className="text-sm font-medium text-muted-foreground">قيمة الأرض الأساسية</dt>
                        <dd className="text-lg font-mono font-semibold text-gray-900" dir="ltr">{formatCurrency(financials.land_base)}</dd>
                    </div>
                    <div className="space-y-1">
                        <dt className="text-sm font-medium text-muted-foreground">نسبة التعويض الإضافي</dt>
                        <dd className="text-lg font-mono font-semibold text-gray-900" dir="ltr">{form.extraCompRate || "0"}%</dd>
                    </div>
                    <div className="space-y-1">
                        <dt className="text-sm font-medium text-muted-foreground">المبلغ الإضافي</dt>
                        <dd className="text-lg font-mono font-semibold text-gray-900" dir="ltr">{formatCurrency(financials.extra_amount)}</dd>
                    </div>
                    <div className="space-y-1">
                        <dt className="text-sm font-medium text-muted-foreground">قيمة التحسينات</dt>
                        <dd className="text-lg font-mono font-semibold text-gray-900" dir="ltr">{formatCurrency(form.improvementValue)}</dd>
                    </div>
                    <div className="space-y-1">
                        <dt className="text-sm font-medium text-muted-foreground">مبلغ الأضرار</dt>
                        <dd className="text-lg font-mono font-semibold text-gray-900" dir="ltr">{formatCurrency(form.damageToRemainingComp)}</dd>
                    </div>
                    <div className="space-y-1">
                        <dt className="text-sm font-medium text-blue-700">المجموع المتوقع</dt>
                        <dd className="text-xl font-mono font-bold text-blue-700" dir="ltr">{formatCurrency(financials.expected_total)}</dd>
                    </div>
                    <div className="space-y-1">
                        <dt className="text-sm font-medium text-muted-foreground">المجموع الرسمي</dt>
                        <dd className="text-lg font-mono font-semibold text-gray-900" dir="ltr">{formatCurrency(financials.official_total)}</dd>
                    </div>
                     <div className="space-y-1">
                        <dt className="text-sm font-medium text-muted-foreground">مجموع التقرير</dt>
                        <dd className="text-lg font-mono font-semibold text-gray-900" dir="ltr">{formatCurrency(financials.report_total)}</dd>
                    </div>
                    <div className="space-y-1">
                        <dt className="text-sm font-medium text-green-700">الأتعاب المحسوبة</dt>
                        <dd className="text-lg font-mono font-bold text-green-700" dir="ltr">{formatCurrency(financials.fee_base)}</dd>
                    </div>
                </div>
            </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader><CardTitle>المستندات المطلوبة</CardTitle></CardHeader>
          <CardContent><div className="space-y-3"><Label>حدد المستندات الناقصة:</Label><div className="space-y-2 max-h-96 overflow-y-auto pr-2">{STANDARD_DOCUMENTS.map((doc) => (<div key={doc.id} className="flex items-start space-x-2 space-x-reverse p-3 rounded-lg hover:bg-accent/50 transition-colors"><Checkbox id={doc.id} checked={selectedDocuments.includes(doc.id)} onCheckedChange={(checked) => { if (checked) { setSelectedDocuments([...selectedDocuments, doc.id]); } else { setSelectedDocuments(selectedDocuments.filter(id => id !== doc.id)); } }} className="mt-1" /><Label htmlFor={doc.id} className="text-sm font-normal leading-relaxed cursor-pointer flex-1">{doc.label}</Label></div>))}{selectedDocuments.length > 0 && (<div className="mt-4 p-3 bg-blue-50 dark:bg-blue-950/20 rounded-lg"><p className="text-sm font-medium text-blue-900 dark:text-blue-100 mb-2">تم تحديد ({selectedDocuments.length}):</p><ul className="text-sm text-blue-800 dark:text-blue-200 space-y-1">{selectedDocuments.map((docId, index) => { const doc = STANDARD_DOCUMENTS.find(d => d.id === docId); return (<li key={docId}>{index + 1}. {doc?.label}</li>);})}</ul></div>)}</div></div></CardContent>
        </Card>
        ...
        <div className="flex gap-4">
          ...
          <Button type="button" variant="outline" onClick={() => setLocation(isEdit ? `/clients/${clientId}` : "/clients")}>
            إلغاء
          </Button>
        </div>
      </form>
