...
export async function getClientByRefCode(refCode: string) {
  ...
  const result = await db
    .select()
    .from(clients)
    .where(eq(clients.refCode, refCode))
    .limit(1);
...
export async function getDashboardStats() {
  ...
  for (const client of allClients) {
    ...
    const area = Number(client.areaSqm) || 0;
    const compensation = Number(client.expectedCompensationTotal) || 0;
...
export async function setSetting(key: string, value: string) {
  ...
  await db
    .insert(settings)
    .values({ key, value } as InsertSetting)
